...
- only authorized inventory roles can create movements
- movement records immutable after insert
- capture `performedBy` for every movement
- optional: include request correlation id in movement metadata

## 18. Testing Strategy

### 18.1 Unit tests
- location matrix policy checks
- adjustment decrease available guard
- reservation/sale linkage rules
- delta snapshot computation

### 18.2 Integration tests (transactional)
- concurrent duplicate idempotency requests
- concurrent transfer/reservation race
- partial reservation and partial sale progression
- rollback on movement insert failure

### 18.3 Regression tests
- ensure existing inventory APIs still read current projection correctly
- ensure movement history filters still work with new fields

## 19. Known Edge Cases and Controls

1. Partial shipments across multiple sales for same reservation.
2. Reservation release after partial sale.
3. Multiple reservations against same order line (allowed or blocked by business policy).
4. Missing inventory record for source location.
5. Duplicate business action with different idempotency keys.
6. Backdated corrections and period-close reporting impact.

## 20. Deployment and Migration Checklist

1. Deploy enum + schema + service + validation changes together.
2. Backfill `idempotencyKey` strategy in clients before enforcing strict production policy.
3. Create unique movement index in controlled migration window.
4. Verify no existing duplicate `idempotencyKey` before unique index apply.
5. Validate movement API contract in Postman/QA suites.
6. Monitor error rates and duplicate-key collisions after release.

## 21. Future Enhancements (Optional)

1. Reservation ledger collection for line-level reservation lifecycle.
2. Optimistic version field on inventory docs for debugging.
3. Outbox pattern for reliable async event delivery.
4. Read models for analytics-heavy reporting.

---

Document owner: Backend/Inventory Engineering
Last updated: 2026-05-11
System: Viteezy V2 Inventory
